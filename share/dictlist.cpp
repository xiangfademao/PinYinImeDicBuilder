...
bool DictList::save_list(FILE *fp) {
  ...
  if (fwrite(buf_, sizeof(char16), start_pos_[kMaxLemmaSize], fp) !=
    ...
    return false;
    printf("保存列表 scis_num_:%d start_pos_:%d start_id_:%d scis_hz_:%d buf_:%x start_pos_[kMaxLemmaSize]:%d\n",scis_num_,start_pos_,start_id_,scis_hz_,buf_, start_pos_[kMaxLemmaSize]);
  return true;
}
...
bool DictList::load_list(FILE *fp) {
  ...
  initialized_ = true;
    printf("读取列表 scis_num_:%d start_pos_:%d start_id_:%d scis_hz_:%d buf_:%x start_pos_[kMaxLemmaSize]:%d\n",scis_num_,start_pos_,start_id_,scis_hz_,buf_,start_pos_[kMaxLemmaSize]);
  return true;
}
}  // namespace ime_pinyin
